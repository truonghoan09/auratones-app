...
import { useAuth } from '../hooks/useAuth'; // Sử dụng lại useAuth
...
interface AuthProps {
  ...
  handleUsernameRegister: (username: string, password: string) => Promise<void>;
...
const Auth = ({
  ...
  const onUsernameLoginClick = async () => {
    await handleUsernameLogin(username, password);
  };

  const onUsernameRegisterClick = async () => {
    await handleUsernameRegister(username, password);
  };
      ...
      <div className="auth-form" onClick={(e) => e.stopPropagation()}>
        {isLoginView ? (
          <>
            ...
            <button onClick={onUsernameLoginClick}>Đăng nhập</button>
          ...
          <>
            ...
            <input type="text" placeholder="Tên người dùng" value={username} onChange={(e) => setUsername(e.target.value)} onKeyDown={handleKeyDown} />
            <input type="password" placeholder="Mật khẩu" value={password} onChange={(e) => setPassword(e.target.value)} onKeyDown={handleKeyDown}/>
            <button onClick={onUsernameRegisterClick}>Đăng ký</button>  
