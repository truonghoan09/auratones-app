...
import LoadingOverlay from "./LoadingOverlay"; // ⬅️ NEW
...
const Auth = ({ showToast, isModal, onClose }: AuthProps) => {
  ...
  const onSubmit = useCallback(async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    setBusy(true); // ⬅️ show overlay khi submit
    try {
      if (isLoginView) {
      ...
      if (mountedRef.current) setBusy(false); // ⬅️ hide overlay
    }
  }, [
      ...
      <form className="auth-form" onSubmit={onSubmit} onClick={(e) => e.stopPropagation()}>
        {isLoginView ? (
          <>
            ...
            <button
              type="button"
              onClick={() => { setBusy(true); handleGoogleAuth(); }} // ⬅️ overlay trước khi redirect
