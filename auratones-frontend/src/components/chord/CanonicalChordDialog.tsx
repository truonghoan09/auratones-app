...
/** Draft state cho canonical dialog */
...
type Props = {
  ...
  onClose: () => void;
  onSubmit: (payload: SubmitPayloadCanonical) => void;
};
...
const ChordCanonicalDialog: React.FC<Props> = ({
  ...
  const validate = () => {
    ...
    if (!allowedRecipeIds.has(normalized)) errors.push("Phân loại hợp âm này chưa có trong canonical list.");
